...
Also, the line absortion profiles are defined here
...
# ---------------------------------------------
def collision_de_exc_rate(wavel, temp, nt, ne):
    # -----------------------------------------
    ...
    for lx in range(prm.nline):
        ...
        coll_rate[lx, :] = 2.0e1 * ne[:] * nt[:] * p_integral[:] * wavel[lx] ** 3 / np.sqrt(temp[:])
...
def compute_optical_depths(nt, temp):
    ...
    if prm.debug_mode:
        ...
        # mass flow rate in Msun/year
        mdot = (4 * const.pi * prm.r_star * prm.r_star * nt[0] * const.year * const.m_p * mu[0] / const.M_sun) * \
               (grid.v_env[0] / const.km_s)
        print(f"\nnt[0] = {nt[0]:.2e} cm^-3, v[0] = {grid.v_env[0]:.2e} km/s, dmu = {mu[0]:.2f}, "
              f"mdot = {mdot:.2e} Msun/year\n")

    # reorder lines according to velocity field
    line_properties_reordered, taur_reordered = reorder_lines(line_properties, taur)

    # compute the line absoprtion profile widths
